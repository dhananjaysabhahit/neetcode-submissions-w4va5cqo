class Solution {
    public int[] minInterval(int[][] intervals, int[] queries) {
        ...
        for(int j=0;j<queries.length;j++){
            System.out.println("querIdx: "+querIdx[j][0]+","+querIdx[j][1]);

            for(int i=0;i<intervals.length;i++){
                System.out.println("interval: "+intervals[i][0]+","+intervals[i][1]);
                if(querIdx[j][0]>=intervals[i][0] && querIdx[j][0]<=intervals[i][1]){
                    int len = intervals[i][1]-intervals[i][0]+1;
                    pq.add(new int[]{len,i});
                } else if(querIdx[j][0]<intervals[i][0]){
                    break;
                }
            }
            if(!pq.isEmpty()){
                queries[querIdx[j][1]]= pq.poll()[0];
                pq.clear();
            } else{
                queries[querIdx[j][1]]= -1;
            }
        }
        return queries;
    }
